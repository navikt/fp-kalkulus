...
class BeregningStegTjenesteTest extends EntityManagerAwareTest {
    ...
    public void beforeEach() {
        ...
        avklaringsbehovTjeneste = new AvklaringsbehovTjeneste(avklaringsbehovRepository, koblingRepository, new AvklaringsbehovKontrollTjeneste());
    }
    ...
    public void skal_gi_aksjonspunkt_om_lønnsendring_vurder_fakta() {
        ...
        var andeler = nyttBG.get().getBeregningsgrunnlag().get()
                .getBeregningsgrunnlagPerioder().get(0).getBeregningsgrunnlagPrStatusOgAndelList();
        assertThat(andeler).hasSize(1);
        assertThat(andeler.get(0).getBgAndelArbeidsforhold().get().getArbeidsforholdOrgnr()).isEqualTo(ORGNR);
    }
    ...
    public void skal_kopiere_gammelt_grunnlag_ved_tilbakerulling() {
        ...
        assertThat(andeler2.get(0).getBgAndelArbeidsforhold().get().getArbeidsforholdOrgnr()).isEqualTo(ORGNR);
    ...
    private void assertResultatOgAktiviteter(TilstandResponse resultat, Optional<BeregningsgrunnlagGrunnlagEntitet> bg, BeregningsgrunnlagTilstand tilstand, boolean medAksjonspunkt, int registerAktiviteter, int saksbehandlerAktiviteter) {
        ...
        assertThat(bg.get().getRegisterAktiviteter().getBeregningAktiviteter()).hasSize(registerAktiviteter);
        assertThat(bg.get().getSaksbehandletAktiviteter().map(BeregningAktivitetAggregatEntitet::getBeregningAktiviteter).orElse(Collections.emptyList())).hasSize(saksbehandlerAktiviteter);
    }
