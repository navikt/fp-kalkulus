...
public class FastsettBeregningsgrunnlagPerioderTjeneste {
    ...
    private MapFastsettBeregningsgrunnlagPerioderFraRegelTilVLUtenAndelendring oversetterFraRegelUtenAndelendring;
    ...
    public FastsettBeregningsgrunnlagPerioderTjeneste(MapFastsettBeregningsgrunnlagPerioderFraVLTilRegelNaturalYtelse oversetterTilRegelNaturalytelse,
                                                      ...
                                                      MapFastsettBeregningsgrunnlagPerioderFraRegelTilVLUtenAndelendring oversetterFraRegelUtenAndelendring) {
        ...
        this.oversetterFraRegelUtenAndelendring = oversetterFraRegelUtenAndelendring;
    }
    ...
    /**
     * Splitter kun perioder og endrer ikkje på andeler (legger heller ikkje til tilkomne andeler)
     *
     * @param input BeregningsgrunnlagInput
     * @param beregningsgrunnlag Beregningsgrunnlag
     * @return Beregningsgrunnlag med splittede perioder
     */
    public BeregningsgrunnlagDto fastsettPerioderUtenAndelendring(BeregningsgrunnlagInput input,
                                                                        BeregningsgrunnlagDto beregningsgrunnlag) {
        var ref = input.getBehandlingReferanse();
        var mapper = FagsakYtelseTypeRef.Lookup.find(oversetterTilRegelRefusjonOgGradering, ref.getFagsakYtelseType())
                .orElseThrow(() -> new IllegalStateException("Finner ikke implementasjon for håndtering av refusjon/gradering for BehandlingReferanse " + ref));
        ...
        PeriodeModell periodeModell = mapper.map(input, beregningsgrunnlag);
        return kjørRegelOgMapTilVLUtenAndelendring(beregningsgrunnlag, periodeModell);
    }
    ...
    private BeregningsgrunnlagDto kjørRegelOgMapTilVLUtenAndelendring(BeregningsgrunnlagDto beregningsgrunnlag, PeriodeModell input) {
        String regelInput = toJson(input);
        logger.info("Regelinput for splitt perioder refusjon og gradering: " + regelInput);
        List<SplittetPeriode> splittedePerioder = FastsettPeriodeRegel.fastsett(input);
        return oversetterFraRegelUtenAndelendring.mapFraRegel(splittedePerioder, regelInput, beregningsgrunnlag);
    }
