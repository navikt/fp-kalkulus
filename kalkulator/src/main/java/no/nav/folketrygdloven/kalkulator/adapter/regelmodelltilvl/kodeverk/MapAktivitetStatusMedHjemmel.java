...
public class MapAktivitetStatusMedHjemmel {
    ...
    public static BeregningsgrunnlagDto mapAktivitetStatusMedHjemmel(List<AktivitetStatusMedHjemmel> aktivitetStatuser,
                                                                     BeregningsgrunnlagDto eksisterendeVLGrunnlag, no.nav.folketrygdloven.kalkulator.regelmodell.resultat.BeregningsgrunnlagPeriode beregningsgrunnlagPeriode) {
        for (no.nav.folketrygdloven.kalkulator.regelmodell.AktivitetStatusMedHjemmel regelStatus : aktivitetStatuser) {
    ...
    private static AktivitetStatus fraRegel(no.nav.folketrygdloven.kalkulator.regelmodell.AktivitetStatus aktivitetStatus, no.nav.folketrygdloven.kalkulator.regelmodell.resultat.BeregningsgrunnlagPeriode beregningsgrunnlagPeriode) {
        ...
        BeregningsgrunnlagPrStatus atfl = beregningsgrunnlagPeriode.getBeregningsgrunnlagPrStatus(no.nav.folketrygdloven.kalkulator.regelmodell.AktivitetStatus.ATFL);
        if (atfl == null) {
        ...
        if (no.nav.folketrygdloven.kalkulator.regelmodell.AktivitetStatus.ATFL.equals(aktivitetStatus)) {
            return mapATFL(frilanser, arbeidstaker);
        } else if (no.nav.folketrygdloven.kalkulator.regelmodell.AktivitetStatus.ATFL_SN.equals(aktivitetStatus)) {
