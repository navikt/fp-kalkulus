...
public class BeregningsgrunnlagRepository {
    ...
    public Optional<BeregningsgrunnlagGrunnlagEntitet> hentSisteBeregningsgrunnlagGrunnlagEntitetForKobling(Long koblingId,
        ...
        if (resultatListe.size() > 1) {
            LOG.info("Fant mer enn ett grunnlag lagret på samme tidspunkt for kobling {}, tar siste id", koblingId);
            return resultatListe.stream().max(Comparator.comparing(BeregningsgrunnlagGrunnlagEntitet::getId));
    ...
    private void lagreGrunnlag(BeregningsgrunnlagGrunnlagEntitet nyttGrunnlag) {
        ...
        AktivitetAggregatEntitet registerAktiviteter = nyttGrunnlag.getRegisterAktiviteter();
        if (registerAktiviteter != null) {
            lagreAktivitetAggregat(registerAktiviteter);
        }
        nyttGrunnlag.getFaktaAggregat().ifPresent(this::lagreFaktaAggregat);
        nyttGrunnlag.getSaksbehandletAktiviteter().ifPresent(this::lagreAktivitetAggregat);
        nyttGrunnlag.getOverstyring().ifPresent(this::lagreAktivitetAggregat);
    ...
    private void lagreRefusjonOverstyring(RefusjonOverstyringerEntitet refusjonOverstyringerEntitet) {
        entityManager.persist(refusjonOverstyringerEntitet);
        refusjonOverstyringerEntitet.getRefusjonOverstyringer().forEach(this::lagreRefusjonOverstyring);

    }

    private void lagreRefusjonOverstyring(RefusjonOverstyringEntitet refusjonOverstyringEntitet) {
        entityManager.persist(refusjonOverstyringEntitet);
        refusjonOverstyringEntitet.getRefusjonPerioder().forEach(entityManager::persist);
    }

    private void lagreAktivitetAggregat(AktivitetAggregatEntitet aggregat) {
        if (aggregat.getId() != null) {
            throw new IllegalArgumentException("Kan ikke lagre et allerede lagret grunnlag");
        }
        entityManager.persist(aggregat);
        aggregat.getAktiviteter().forEach(entityManager::persist);
    }
